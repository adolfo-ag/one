...
class _WorldBlueprint {
  const _WorldBlueprint({
    required this.id,
    required this.name,
    required this.icon,
    required this.color,
...
const List<_WorldBlueprint> _worldBlueprints = [
  _WorldBlueprint(
    id: 'world_1',
...
final worldCatalogProvider = Provider<List<WorldDefinition>>((ref) {
  final lessonsByWorld = ref.watch(lessonCatalogProvider);

  return _worldBlueprints
      .map(
        (blueprint) => WorldDefinition(
          id: blueprint.id,
          name: blueprint.name,
          icon: blueprint.icon,
          color: blueprint.color,
          lessonCount: lessonsByWorld[blueprint.id]?.length ?? 0,
        ),
      )
